...
def generate_report(sheet_id):
    ...
    # Detecta coluna de data automaticamente em cada DataFrame
    def find_date_col(df):
        for candidate in ['Data', 'Dia', 'Date', 'data', 'dia', 'date', 'DATA']:
            if candidate in df.columns:
                return candidate
        # fallback: primeira coluna que contenha 'dat' no nome
        for c in df.columns:
            if 'dat' in c.lower() or 'dia' in c.lower():
                return c
        return None
    ...
    for df in [df_kommo, df_meta, df_google]:
        ...
        if date_col:
            df['Data format'] = pd.to_datetime(df[date_col], dayfirst=True, errors='coerce')
        else:
            df['Data format'] = pd.NaT
